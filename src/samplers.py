...
class IntSampler(DataSampler):
    ...
    def sample_xs(self, n_points, b_size, n_dims_truncated=None, seeds=None):
        ...
        def reandom_numbers(n, generator=None, resort=False):
            arr = None
        ...
        if seeds is None:
            for i in range(b_size):
                xs_b[i] = reandom_numbers(n_points * self.n_dims, resort=self.resort).reshape(n_points, self.n_dims)
        else:
            generator = torch.Generator()
            assert len(seeds) == b_size
            for i, seed in enumerate(seeds):
                generator.manual_seed(seed)
                xs_b[i] = reandom_numbers(n_points * self.n_dims, generator=generator, resort=self.resort).reshape(n_points, self.n_dims)
